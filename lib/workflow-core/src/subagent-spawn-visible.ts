...
import { injectParentSession } from "./subagent-utils";

/** 关闭 tmux 窗格 */
function closeTmuxPane(paneId: string): void {
	try { execSync(`tmux kill-pane -t ${paneId}`, { stdio: "pipe" }); } catch { /* ignore */ }
}
...
export function spawnVisible(
	...
	return new Promise<SubagentResult>((resolve) => {
		...
		/** 在 resolve 前注入 parentSession */
		const resolveWithInjection = (result: SubagentResult) => {
		...
		const pollInterval = setInterval(() => {
			...
			if (Date.now() - lastActivityTime > STALL_MS) {
				...
				resolveWithInjection({ exitCode: 1, output, stderr: "", timedOut: true, subSessionId: capturedSessionId });
			}
		}, 2000);
		...
		function finish() {
			...
			resolveWithInjection({ exitCode: 0, output, stderr: "", subSessionId: capturedSessionId });
		}
		...
		if (signal) {
			const kill = () => {
				if (!resolved) {
					...
					closeTmuxPane(paneId);
					try { fs.rmSync(taskDir, { recursive: true }); } catch { /* ignore */ }
					resolveWithInjection({ exitCode: 1, output: allTexts.join("\n\n---\n\n") || "", stderr: "aborted", subSessionId: capturedSessionId });
				}
			};
