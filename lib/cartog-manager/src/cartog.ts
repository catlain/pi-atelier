...
/** 合并全局 + 项目配置，展开路径，去重 */
export function resolveExtraDirs(cwd: string, globalCfgPath?: string): string[] {
	const globalCfg = loadConfig(globalCfgPath ?? GLOBAL_CONFIG_PATH);
 ...
 * 用于 --watch 模式下 cartog_index 拦截器快速路径。
 * 返回 { changed } 表示链接是否有变化。
 */
export function syncSymlinksOnly(cwd: string, globalCfgPath?: string): { changed: boolean } {
	const extDir = join(cwd, CARTOG_EXT_DIR);
	const extraDirs = resolveExtraDirs(cwd, globalCfgPath);
	...
	try {
		for (const entry of readdirSync(extDir)) existingEntries.add(entry);
	} catch { /* ignore */ }

	for (const entry of existingEntries) {
		if (!desiredLinks.has(entry)) {
			try { rmSync(join(extDir, entry), { recursive: true, force: true }); changed = true; } catch { /* ignore */ }
		}
	}
	...
	for (const [name, target] of desiredLinks) {
		...
		try {
			if (lstatSync(linkPath).isSymbolicLink()) {
				if (realpathSync(linkPath) === resolve(target)) continue;
				unlinkSync(linkPath);
			} else {
				continue;
			}
...
/**
 * 从已有 .cartog.db 读取文件/符号统计。
 * 用于 db 存在但不需要重新索引的场景。
 */
export function getDbStats(dbPath: string): { fileCount: number; symbolCount: number } {
	let fileCount = 0;
	let symbolCount = 0;
	try {
		const stats = JSON.parse(safeExec(`cartog --db "${dbPath}" stats --json`) || "{}");
...
export function buildProjectIndex(cwd: string): IndexResult {
	...
	// ── 同步软链接（复用 syncSymlinksOnly）──
	const { changed } = syncSymlinksOnly(cwd);

	// ── 判断是否需要索引 ──
	const needsIndex = changed || !existsSync(projectDb);

	if (!needsIndex) {
		const { fileCount, symbolCount } = getDbStats(projectDb);
		return { indexed: false, fileCount, symbolCount };
	}

	// ── 从项目根目录索引（cartog 会跟随 cartog-ext/ 下的软链接） ──
	console.log(`[cartog] 从项目根索引: ${cwd} → ${projectDb}`);
	const output = safeExec(`cartog index "${cwd}" --db "${projectDb}" --force`, 180_000);
