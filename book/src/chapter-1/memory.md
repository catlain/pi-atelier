...
此外，记忆系统还有**冲突检测**——如果新文件与已有记忆的 topic 相同或关键词重叠 3 个以上，会**直接拒绝写入**，强制先处理冲突（合并或覆盖）。
...
4. 用 `memory_update` **覆盖写入到已有文件的文件名**（而不是新建文件名，否则会触发冲突检测被拒绝）
5. 删除旧的碎片文件
...
**原则**：
- **项目特有**的知识（这个项目的架构、约定）→ 项目级 `.pi/memory/`
