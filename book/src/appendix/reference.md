...
| pi-context-manager | `"pi-context-manager"` | distill + 过滤 + `payload_analyze` | 上下文质量控制 + Token 诊断 |
...
工具返回的结果可能很大（比如读取一个 1000 行的文件）。pi 内置了 distill 机制来压缩工具输出。pi-context-manager 扩展可以自定义 distill 策略，同时提供 payload_analyze 工具诊断 token 消耗。
...
3. 执行工具
     │
     ▼
4. Context Manager distill 处理返回值
     │
     ▼
...
```
.pi/
├── distill/         # pi-context-manager 的 distill 数据
