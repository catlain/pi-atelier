...
|------|------|------|
| `comment` | ✅ | 规则注释,方便理解 |
| `hook` | ✅ | 触发时机：`tool_call` / `tool_result` / `agent_end` / `session_shutdown`（默认 `tool_call`） |
| `tool` | ❌ | 限定只匹配特定工具（如 `"edit"`、`"bash"`、`"grep"`，默认 `"bash"`） |
| `action` | ✅ | 动作：`block` / `notify` / `rewrite` / `steer`（默认 `block`） |
| `conditions` | ❌ | 条件数组,所有条件同时满足才触发 |
| `pattern` | ❌ | 正则匹配(匹配工具参数或命令内容) |
| `reason` | ✅ | 注入给 AI 的提示文本(`notify`/`steer` 动作时) |
| `state` | ❌ | 状态追踪(如累计错误次数) |
| `check` | ❌ | agent_end/session_shutdown 专用检查：`has_edits`（有编辑）/ `git_uncommitted`（有未提交）/ `always`（始终触发） |
